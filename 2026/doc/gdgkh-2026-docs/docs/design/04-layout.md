...
- **Hero 區採兩欄 grid**：左欄放活動資訊（標題、日期、地點、購票 CTA、倒數計時），
  ...
  詳細規格見 D33。
- **Hero 雙欄斷點自 2026-08-04 由 1280px 下修至 900px**（D33 追加）：
  原本 1280px 才啟用雙欄，導致中型筆電、視窗未最大化、或 `Cmd+` 放大字級時
  頻繁塌陷成單欄，與截圖版型不符。下修至 900px 後涵蓋 13" 筆電內縮視窗、
  iPad 橫向與多數縮放情境；iPad 直立（<900px）維持單欄，等同手機模式。
  斷點下修只影響 `.gk-hero`、`.gk-hero-inner`、`.gk-ballot-card` 三處樣式，
  導覽列與其他 section 的 1280px 斷點不動。
...
「特別感謝」在 2026-08-04（D33）改由 Hero 右欄同時承接視覺呈現：
點導覽列的「特別感謝」時，桌機隱藏其他 section 只留 Hero，手機捲到 Hero 內的
`#gk-hero-thanks`。項目本身保留、不從導覽列移除。
