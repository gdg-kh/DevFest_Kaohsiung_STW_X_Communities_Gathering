...
- 購票入口共兩處：導覽列、Hero 區，GA 事件都帶 `entry` 參數區分
- 點選單切換 section（hash route，例如 `#/speakers`），一次只顯示一個區塊
- 「去年頁面」直接開新分頁
- **Hero 桌機雙欄**：≥ 900px 時左欄放活動名稱／日期／地點／購票按鈕／倒數計時，
  右欄放特別感謝跑馬燈；< 900px 時右欄折至下方，維持單欄堆疊
...
**一、先從結構減量。** 把「去年頁面」移出導覽列，改成首頁的快速入口卡片（見 6.7）；
「主辦單位」併入活動介紹的一段（見 6.7、6.8）。頁尾同時保留一份純文字連結。
...
**四、再放不下就進手機模式。** 桌機門檻設在 **900px**，
900px 以下直接切到一頁式 + 可橫向捲動的 chip 導覽，且 Hero 右欄的跑馬燈折至下方。
這段程式本來就要寫，等於免費多一層保險。
...
- 「購票」維持在 Hero 區；「去年頁面」是首頁卡片
- Hero 右欄的跑馬燈折至 Hero 下方
- 右下角回到頂部按鈕（做成競選旗幟造型）
...
- 除了頁尾之外，也在 `about.sections.vision` 的內文尾端提醒一句，
  讓人在報名前就看得到
...
排版乾淨可讀（畢竟是要看議程的），但視覺記憶點大膽——超粗黑體、紅色印章、宣傳車跑馬燈、「凍蒜」感嘆號、選票勾選框。梗放在文案與微互動，不犧牲資訊可讀性。

具體玩梗點（可再增減）：
- Hero：「2026 高雄開發者大選」「政見發表會」，倒數計時做成「開票倒數」
- 講者卡：圓形頭像＋紅框，議程名稱當「競選標語」
- 議程群組篩選 chip = 「政黨」色籤
- 購票按鈕：「投他一票」＋點下去蓋章動畫
...
| 講者卡 | 候選人看板：圓形頭像＋紅框、姓名大字、議程名稱當競選標語 |
...
| 免費票申請 | 併入 `about.sections.free_ticket` 一段純文字，含資格、審核流程與 Google 表單連結；網站不碰優惠碼 |
| 虛擬會場 | 線上競選總部 |
| 頁尾 | 選舉公報風：印章、聲明小字 |
...
| 導覽列 | 保證單行：結構減量 + 流體字級（下限 14px）+ 量測式溢出選單；桌機門檻 900px |
| 去年頁面 | 移出導覽列，改成首頁快速入口卡片，頁尾另留純文字連結 |
| 主辦單位 | 併入 `about.sections.organizer` 一段純文字，不做首頁卡片或彈窗 |
...
2. 2025 講者資料有 `org`（公司）與 `title`（職稱），2026 已納入資料模型（見 §5 Speaker），
   卡片與詳細彈窗會顯示為「title · org」。
3. 購票平台是 KKTIX 還是其他？
